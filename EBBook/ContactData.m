...
+(ABContact *) byPhoneNumberAndNameToGetContact:(NSString *)name withPhone:(NSString *)phone{
    ...
    for(ABContact * contast in array)
    {
        ...
        {
            NSArray *numArray = [contast phoneDictionaries];
            for (NSDictionary *dic in numArray) {
                NSString *mobile = [dic objectForKey:@"value"];
...
+(NSArray *) getPhoneNumberAndPhoneLabelArrayFromABRecodID:(ABRecordRef)person withABMultiValueIdentifier:(ABMultiValueIdentifier)identifierForValue
{
	...
	NSMutableDictionary *phoneDic = [NSDictionary dictionaryWithObjectsAndKeys:nameStr,KPHONENAMEDICDEFINE,phone,KPHONENUMBERDICDEFINE,label,KPHONELABELDICDEFINE,nil];
	[phoneArray addObject:phoneDic];
	CFRelease(phoneMulti);
	return phoneArray;
}
